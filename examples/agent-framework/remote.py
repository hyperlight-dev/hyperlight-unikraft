...
A real `agent_framework` Agent backed by a custom ChatClient that calls the
GitHub Models inference API (OpenAI-compatible) over the network. It runs
inside a Hyperlight micro-VM with networking enabled; the API token is passed
into the guest by `hluk run --env GITHUB_TOKEN=...` — never baked into the
image or written to disk.

Agent Framework ships `OpenAIChatCompletionClient`, but that uses the async
OpenAI SDK. This example uses a small synchronous `urllib` client and steps the
coroutine directly instead of `asyncio.run()`, which keeps it free of an event
loop (whose self-pipe needs `socket.socketpair()`, absent from the unikernel).
"""
...
MAX_COMPLETION_TOKENS = int(os.environ.get("GITHUB_MODELS_MAX_COMPLETION_TOKENS", "128"))
...
class GitHubModelsChatClient(BaseChatClient):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if not self._token:
            raise RuntimeError(
                "GITHUB_TOKEN must be set on the host and passed with "
                "hluk run --env GITHUB_TOKEN=..."
            )
    ...
    def _inner_get_response(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise RuntimeError(f"GitHub Models API returned HTTP {exc.code}: {message}") from exc
