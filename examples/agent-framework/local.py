...
A real `agent_framework` Agent backed by a custom ChatClient that runs a small
GGUF model with llama-cpp-python. The model is baked into the rootfs, so
inference happens entirely inside the Hyperlight micro-VM — no network access
and no API keys.
"""
...
# llama-cpp-python imports multiprocessing (via llama.py) and sqlite3 (via
# diskcache) transitively. This example uses neither; register minimal stubs so
# the imports succeed even on a rootfs that trims them. `setdefault` is a no-op
# when the real modules are present.
...
class LlamaCppChatClient(BaseChatClient):
    ...
    def _inner_get_response(
        ...
        **kwargs: Any,
        ...
        chat = [{"role": m.role, "content": m.text} for m in messages if m.text]
...
def run_sync(coro: Coroutine[Any, Any, Any]) -> Any:
    ...
    `asyncio.run()` can't be used here: it builds a selector event loop whose
    wake-up self-pipe needs `socket.socketpair()`, which this unikernel doesn't
    implement (ENOSYS). The inference call is synchronous and never suspends.
    """
    try:
        while True:
...
async def main() -> None:
    ...
    query = "In one sentence, what is a micro-VM?"
    print(f"User:  {query}")
