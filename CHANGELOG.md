...
- Loading a snapshot saved by another release fails with `Error::SnapshotRelease`, which names that release and this one and says to save it again, in `hluk snapshot run`, `hluk bench` and the library alike. A snapshot works only with the release that saved it; the OCI layout's own "no manifest tagged" error said neither.
